...
<template>
  <v-container class="mx-auto">
    ...
    <QuizQuestions :questions="questionsList" :readOnly="true" />
    <div
      v-if="addQuestion"
    >
...
export default {
  ...
  methods: {
    updateQuestionsList(ques) {
      const questionIndex = this.questionsList.findIndex((x) => x.question.id === ques.question.id);
      if (questionIndex >= 0) {
        this.$set(this.questionsList, questionIndex, ques);
      } else {
        this.questionsList.push(JSON.parse(JSON.stringify(ques)));
      }
      this.addQuestion = false;
      this.$root.$emit('selected-question', null);
      window.console.log(ques, this.addQuestion, this.questionsList);
    },
